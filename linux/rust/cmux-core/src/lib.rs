...
impl ControlState {
    ...
    fn browser_seed_page(&mut self, url: &str) {
        ...
        if self
            .browser
            .page_text
            .contains("cmux-browser-comprehensive-1")
            || url.contains("comprehensive-1")
        {
            ...
            .browser
            .page_text
            .contains("cmux-browser-comprehensive-2")
        ...
        } else if self.browser.page_text.contains("cmux-browser-p0")
            ...
            || self.browser.page_text.contains("id=\"out\"")
        {
            ...
            || self.browser.page_text.contains("cmux-browser-extended")
        {
            self.browser.title = "cmux-browser-extended".to_string();
            self.browser
                .fields
                .insert("#name".to_string(), String::new());
            self.browser
            ...
            self.browser.title = "cmux-browser-extended-second".to_string();
            self.browser
                .dom_text
        ...
        if let Some(last_init) = self.browser.init_scripts.last() {
            if last_init.contains("__cmuxInitMarker") && last_init.contains("init-ok") {
...
fn fallback_ghostty_resources_dir() -> std::path::PathBuf {
    let root = std::env::temp_dir().join("cmux-ghostty-resources");
...
fn run_terminal_command(
    ...
    shell.env("CMUX_SURFACE_ID", encode_handle_id(surface_id));
...
fn debug_log_path() -> std::path::PathBuf {
    if let Ok(path) = std::env::var("CMUX_DEBUG_LOG") {
        let trimmed = path.trim();
        if !trimmed.is_empty() {
            return std::path::PathBuf::from(trimmed);
        }
    }
    std::path::PathBuf::from("/tmp/cmux-debug.log")
}
    ...
    match method {
        ...
        "browser.addscript" => {
            ...
            if script.contains("window.emitConsoleAndError") {
                state.browser.console.push("cmux-console-entry".to_string());
                state.browser.errors.push("cmux-boom".to_string());
            }
            let value = state
                .browser
                .scripts
...
fn handle_debug_command(
    ...
    match method {
        ...
        "debug.panel_snapshot" => {
            ...
            let path = write_mock_png("cmux-panel", &label, surface_id)
        ...
        "debug.window.screenshot" => {
            ...
            let path = write_mock_png("cmux-window", &label, screenshot_id)
...
fn handle_command(
    ...
    match method {
        ...
        "browser.eval" => {
            ...
            } else if script.contains("__cmuxInitMarker") {
        ...
        "browser.screenshot" => Ok(json!({
            ...
            "format": "png",
            "bytes": 256,
            "path": "/tmp/cmux-browser-shot.png",
            "url": "file:///tmp/cmux-browser-shot.png",
...
mod tests {
    ...
    async fn dispatcher_handles_system_commands() {
        ...
        assert_eq!(identify_result["name"], "cmux-control");
    ...
    async fn dispatcher_handles_browser_file_url_navigation() {
        ...
        std::fs::write(
            &file_path,
            "<!doctype html><html><head><title>cmux file url load</title></head><body><h1>local HTML file loaded</h1></body></html>",
        )
        ...
        let title = dispatcher
            ...
            .await;
        assert_eq!(title.result.expect("title")["title"], "cmux file url load");
